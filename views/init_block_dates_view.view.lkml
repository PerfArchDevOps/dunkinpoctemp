view: init_block_dates_view {
    derived_table: {
      sql: SELECT TRUNC (sysdate) today_date,
      TRUNC (sysdate, 'YEAR') ytd_start
          ...
          TRUNC (sysdate) - 1 br_yesterday,
           ...
           , TRUNC (LAST_DAY (sysdate - 45 - 30)) fps_lpdate
           , month_short_name || '
      ...
      (week_ending_date) - 84 prior_12_week,
      case when TO_NUMBER(TO_CHAR(sysdate,'MM')) < 3 THEN TRUNC (sysdate-365, 'YEAR') ELSE TRUNC (sysdate, 'YEAR')  END ops_prompt_variable,
      case when week_of_year = 1 then to_char(TRUNC(WEEK_ENDING_DATE) - (WEEK_OF_YEAR * 7), 'YYYY-MM-DD') else to_char(TRUNC(WEEK_ENDING_DATE) - (WEEK_OF_YEAR * 7 - 7), 'YYYY-MM-DD') end rpm_first_week_of_year,
      case when week_of_year = 1 then to_char(TRUNC(ly_WEEK_ENDING_DATE) - (WEEK_OF_YEAR * 7), 'YYYY-MM-DD') else to_char (TRUNC(ly_WEEK_ENDING_DATE) - (WEEK_OF_YEAR * 7 - 7), 'YYYY-MM-DD') END rpm_forst_week_ly,
